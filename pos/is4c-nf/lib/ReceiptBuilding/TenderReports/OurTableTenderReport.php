...
/**
  @class OurTableTenderReport
  Custom tender report for Our Table Cooperative
  Based on WefcTorontoTenderReport (forked 4/7/2019)
  Reports on ALL tenders, ignores TRDesiredTenders in Lane Install-Extra settings
  Reports for the entire current day.
  Reports from localtranstoday if offline.
*/
class OurTableTenderReport extends TenderReport {
...
static public function get($session){
    ...
    //Check for suspended transactions
    if (SuspendLib::checksuspended($session)) {
        $localCheck .= "\n\nTHERE ARE SUSPENDED TRANSACTIONS!\n";
        $localCheck .= "**** PLEASE CLOSE THESE TRANSACTIONS OR CANCEL THEM. ****\n\n";
    }
    ...
    if (MiscLib::pingport($CORE_LOCAL->get("mServer"), $CORE_LOCAL->get("mDBMS"))) {
        ...
        $res = $db_p->query("SELECT TenderName, TenderCode FROM tenders ORDER BY TenderName");
        while($row = $db_p->fetch_row($res)) {
              $DESIRED_TENDERS[$row['TenderCode']] = $row['TenderName'];
        }
...
*/
    ...
    $receipt .= ReceiptLib::centerString("O U R  T A B L E  T E N D E R   R E P O R T")."\n";
    ...
    $receipt .= self::trTotal('MI','STORE CREDITS');
