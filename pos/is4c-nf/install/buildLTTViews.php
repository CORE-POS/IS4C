...
$createStr = "CREATE view lttsummary as
	...
	convert(sum(case when discounttype=4 THEN memDiscount ELSE 0 END), decimal(10,2)) as staffSpecial,
...
while ($taxRatesW = $db->fetch_row($taxRatesR)){
	$createStr .= "convert(sum(case when (trans_type = 'I' or trans_type = 'D') and tax = ".$taxRatesW[0]." and discountable = 0 then total else 0 end),decimal(10,2)) as noDiscTaxable_".$taxRatesW[1].",\n";
...
convert(sum(case when foodstamp = 1 and discountable = 0 then total else 0 end),decimal(10,2)) as fsNoDiscTTL,
convert(sum(case when foodstamp = 1 and discountable <> 0 then total else 0 end),decimal(10,2)) as fsDiscTTL,
...
CONVERT(fsTendered + fsNoDiscTTL + (fsDiscTTL * ((100-percentDiscount)/100)), DECIMAL(10,2)) AS fsEligible,\n";
if(count($rates) > 0){
	for ($i = 0; $i < count($rates); $i++){
		$createStr .= "convert((fsDiscTaxable_".$desc[$i]."*((100-percentDiscount)/100)) + fsTaxable_".$desc[$i].",decimal(10,2)) as fsTaxable_".$desc[$i].",";
		$createStr .= "convert((fsDiscTaxable_".$desc[$i]."*((100-percentDiscount)/100)*".$rates[$i].")+(fsTaxable_".$desc[$i]."*".$rates[$i]."),decimal(10,2)) as fsTax_".$desc[$i].",\n";
	}
}
