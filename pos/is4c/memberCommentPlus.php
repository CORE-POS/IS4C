...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <style type='text/css'>
            ...
            .header {
                text-align: left; background-color: #eeeeee; font-weight: bold; color: #000000;
            }
            .skinny {
            ...
            if (!pinghost($_SESSION["memServer"])){
                ...
                if (strlen($memNum) == 6){
                    $query="select Member, Last_Name, First_Name, Last, First, Address, Address2, City, State, Zip,"
                        ...
                        . "from MBRData where Member='" . $memNum . "'";
                    ...
                    $query="select convert(char(8),U.[Date],1), m.Purchase, PerVisitCoupons, sum(-1*(U.Amount)) as CouponValue"
                        . "from (select Member,[Date], Register_No, Emp_No, Trans_No, Sum(CouponsUsed) as PerVisitCoupons,"
                        . "-1*sum([Total]) as Amount"
                        . "from ".$_SESSION["remoteDB"]."CouponUsagePolled where member=" . $memNum
