...
if (file_exists($remote_oux)){
    ...
    elseif ($Result == "Error") {
        if($Reference == "Invalid Card Num"){
            $_SESSION["boxMsg"] = "Invalid card Number. Please try again<p>or [clear] to cancel</p>";
            //-------added 04/01/05 by CvR---need to save TroutD to void it
            ccout($User1,$Result,$TroutD,0,$catTroutD);
        }
        elseif ($Reference == "F+Card# to Force"){
            $_SESSION["boxMsg"] = "This appears to be a <br />duplicate transaction.<br />Enter 'F' before swiping<p>or [clear] to cancel</p>";
            ccout($User1,$Result,$TroutD,0,$catTroutD);
        }
        else {
   ...
   //-----
    ...
    elseif ($Result == "NOT CAPTURED" && $Reference == "TRY AGAIN"){
        $_SESSION["ccTender"] = 0;
        $_SESSION["boxMsg"] = "Communication error, please try again.<p>[clear]</p>";
        boxMsgScreen();
        //-------added 04/01/05 by CvR---need to save TroutD to void it
        ccout($User1, $Result, $TroutD, 0, $catTroutD);
        //-----
    ...
    elseif($Result == "NOT CAPTURED"){
        ...
        $_SESSION["boxMsg"] = "Unable to process<br />" . $errorInfo . "<p>[clear]</p>";
