...
while(!feof($fp)){
	...
	$tmp = strtoupper($data[$QTY]);
	if (substr($tmp,0,3)=="EA " || substr($tmp,0,3)=="CS ")
		$tmp = substr($tmp,3);
	elseif (substr($tmp,0,4) == "EA* ")
		$tmp = substr($tmp,4);
	$tmp = explode("/",$tmp);
	$qty = $tmp[0];
	$size = $tmp[1];
...
if (count($filestoprocess) == 0){
	...
	echo "Finished processing SELECT price file<br />";
	if ($PRICEFILE_USE_SPLITS){
	...
	header("Location: loadSELECTprices.php?filestoprocess=$sendable_data&processed=$encoded2");
