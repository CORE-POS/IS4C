...
if (!validateUserQuiet('view_all_hours')){
	header("Location: {$FANNIE_URL}auth/ui/loginform.php?redirect={$FANNIE_URL}legacy/it/hours/weeklyHours.php");
	return;
}
...
if (isset($_POST["empID"])){
	...
	$checkR = $sql->query("SELECT empID FROM EmpWeeklyNotes WHERE empID=$empID");
	if ($sql->num_rows($checkR) == 0){
		$sql->query("INSERT INTO EmpWeeklyNotes VALUES ($empID,'$notes')");
	}
	else{
		$sql->query("UPDATE EmpWeeklyNotes SET notes='$notes' WHERE empID=$empID");
	}
}
...
$query = "SELECT e.name,e.adpID,d.name,n.notes,
	...
	WHERE datediff(w.weekStart,'$limitDay') >= 0
	AND datediff(w.weekStart,'$limit2') <= 0
	AND e.empID=$empID
