...
if (!validateUserQuiet('upload_hours_data')){
	header("Location: {$FANNIE_ROOT}auth/ui/loginform.php?redirect={$FANNIE_ROOT}legacy/it/hours/importWeekly.php");
	return;
}
...
elseif (isset($_POST["data"])){
	...
	foreach($_POST["data"] as $d){
		...
		$res = $db->query("SELECT * FROM fullTimeStatus WHERE empID=$empID");
		if ($db->num_rows($res) == 0){
			$q = "INSERT INTO fullTimeStatus VALUES ($empID,'$status')";
			$db->query($q);
		}
		else {
			$q = "UPDATE fullTimeStatus SET status='$status' WHERE empID=$empID";
			$db->query($q);
		}

		$q = "INSERT INTO weeklyHours VALUES ('$start','$end',$empID,$hours)";
		$db->query($q);
	}
