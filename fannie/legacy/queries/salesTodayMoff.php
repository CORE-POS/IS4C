...
sum(case when d.register_no = 30 then total else 0 end) as prodSales,
sum(total)as Sales
FROM $dlog as d 
WHERE datediff(dd,tdate,'2008-08-02')=0
AND (trans_type ='I' OR Trans_type = 'D' or trans_type='M')
GROUP BY datepart(hh,tDate)
order by datepart(hh,tDate)";

$query2="SELECT 
sum(case when d.register_no=30 then total else 0 end) as produceTotal,
sum(total) as TotalSales
FROM $dlog as d 
WHERE datediff(dd,'2008-08-02',tDate)=0
AND (trans_type ='I' OR Trans_type = 'D' or trans_type='M')";
...
echo "<center><h1>Today's <span style=\"color:green;\">MOFF</span> Sales!</h1>";
echo "<table>";
...
echo "<tr><td>$row2[0] (".round($row2[0]/$row2[1]*100,2)."%)</td></tr>";
echo "</table>";
