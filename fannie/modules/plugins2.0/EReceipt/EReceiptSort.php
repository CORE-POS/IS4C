    ...
    */
    public function sort(array $rowset)
    {
        ...
        if ((count($items['_uncategorized']) > 0)) {
            usort($items['_uncategorized'],array('EReceiptSort','recordCompare'));
        ...
        if (count($headers) > 0) {
            ...
            foreach($headers as $hrow) {
                if (count($items[$hrow['description']]) > 0) {
                    $returnset[] = $hrow;
                    usort($items[$hrow['description']],array('EReceiptSort','recordCompare'));
        ...
        // finally tenders
        if(count($tenders) > 0) {
            usort($tenders, array('EReceiptSort','recordCompare'));
