    ...
    if (isset($_GET['id'])) {
        foreach ($_GET['id'] AS $key => $id) {
            ...
            if (!$result) {
                ...
                echo "<p>MySQL Error: " . mysql_error($db_master) . "</p>";
            }
...
if (isset($_GET['function']) && isset($_GET['emp_no']) && isset($_GET['periodID'])) {
    ...
    if ($emp_no > 0) {
        $mainQ = "SELECT date, DATE_FORMAT(date, '%M %D'), ROUND(SUM(TIMESTAMPDIFF(MINUTE, time_in, time_out))/60, 2)
            ...
            GROUP BY date";
        $mainR = mysql_query($mainQ, $db_master);
        ...
        while ($mainRow = mysql_fetch_row($mainR)) {
            ...
            while ($row = mysql_fetch_row($result)) {
                if ($row[2] == 0) {
                    ...
                    $shiftQ = "SELECT * FROM is4c_log.shifts WHERE ShiftID NOT IN (0,31) ORDER BY ShiftID ASC";
                    $shiftR = mysql_query($shiftQ, $db_master);
       ...
       /*  $houseChargeQ = "SELECT ROUND(SUM(d.total),2)
        ...
        $houseChargeR = mysql_query($houseChargeQ, $db_master);
