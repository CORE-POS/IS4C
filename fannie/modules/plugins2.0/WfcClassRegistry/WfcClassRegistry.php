...
*/
class WfcClassRegistry extends FanniePage 
{
    ...
    private function listClasses()
    {
        ...
        if ($key) {
            ...
            foreach ($items->find() as $item) {
                ...
                if ($item->first_name()) {
                    $ret .= sprintf('
                        <td><a class="btn btn-default" href="%s?class_plu=%d&id=%d&cancel=1&key=%d">Cancel</button></td>',                
                        $FANNIE_ROOT,
                        $item->upc(),
                        $item->id(),
                        $key
            ...
            foreach ($items->find() as $item) {
                ...
                if ($item->first_name()) {
                    $ret .= sprintf('
                        <td><a class="btn btn-default" href="%s?class_plu=%d&id=%d&sign_pay=1&key=%d">Sign / Pay</button></tr>',
                        $FANNIE_ROOT,
                        $item->upc(),
                        $item->id(),
                        $key
    ...
    private function cancel_vew()
    {
        ...
        $ret .= '<a class="btn btn-default" href=' . $FANNIE_ROOT . '?class_plu=' . $key . '>Return to Registry</a>';
        return $ret;
    }
