...
class DeliCateringOrdersPage extends FannieRESTfulPage 
{
    ...
    public function get_review_view()
    {
        ...
        while ($row = $dbc->fetch_row($result)) {
            $ret .= '<tr><td width="150px" align="right"><b>Date Order Placed</b></td><td>' . $row['order_date'] . '</td>';
            $ret .= '<tr><td align="right"><b>Owner #</b></td><td>' . $row['card_no'] . '</td>';
            $ret .= '<tr><td align="right"><b>Name</b></td><td>' . $row['name'] . '</td>';
            $ret .= '<tr><td align="right"><b>Phone</b></td><td>' . $row['phone'] . '</td>';
            $ret .= '<tr><td align="right"><b>Alt. Phone</b></td><td>' . $row['alt_phone'] . '</td>';
            $ret .= '<tr><td align="right"><b>Email</b></td><td>' . $row['email'] . '</td>';
            $ret .= '<tr><td align="right"><b>Notes</b></td><td>' . $row['notes'] . '</td>';
    ...
    public function get_id_confirm_view()
    {
        ...
        foreach ($_GET as $key => $value) {
            ...
            if ($_GET['card_no']) $card_no = $_GET['card_no'];
            if ($_GET['phone']) $phone = $_GET['phone'];
            if ($_GET['alt_phone']) $alt_phone = $_GET['alt_phone'];
            if ($_GET['email']) $email = $_GET['email'];
            if ($_GET['member']) $member = $_GET['member'];
            if ($value > 0 && substr($key,0,2) == 'id') {
                $curStr = $key;
                $order[substr($curStr, 2, 3)] = $value;
                $upc = substr($curStr, 2, 3);
                $this->orders[$upc] = $value; 
            /*
            } elseif (is_string($value) && substr($key,0,2) == 'id'
                && $value != NULL) {
                $curStr = $key;
                $order[substr($curStr, 2, 3)] = 1;
                $upc = substr($curStr, 2, 3);
                $this->orders[$upc] = 1; 
            */
            } elseif ($value === 'LG') {
                $curStr = $key;
                $order[substr($curStr, 2, 3) + 1] = 1;
                $upc = substr($curStr, 2, 3) + 1;
                $this->orders[$upc] = 1; 
            } elseif (substr($key,0,2) == 'op') {
                if ($_GET[$upc] == 0) {
                    $order[$upc + 1] = $value;
                    $this->orders[$upc + 1] = $value;
                } else {
                $order[$key] = $value;
        ...
        $args2 = array($name, $card_no, $phone, $alt_phone, $email, $order_num, $date, $status, $notes, $member);
        foreach ($this->orders as $key => $value) {
            ...
            $args = array($key, $value, $order_num);
            $args3 = array($value, $order_num, $thisUpc);
            
            if (strlen($key) == 3 && is_numeric($key)) {
                $query = $dbc->prepare("
        ...
        $query = $dbc->prepare("
                ...
                ");
        $result = $dbc->execute($query, $args2);
            ...
            <div class="form-inline" >
                <h2>Customer Info</h2><br>
                    <div class="container">
                        <ul style="list-style-type:square">
                            <li>WFC Owners receive a 10% discount on pre-order Deli prepared foods catering orders totalling more than $150.</li>
                ...
                <div class="container">
                    ...
                    if ($_GET['member'] < 1) $ret .= ' checked> Non-Owner<br>';
                        else $ret .= '> Non-Owner<br>';
                        
                    $ret .= '
                    <input type="radio" class="form-control" name="member" value="1"';
                    if ($_GET['member'] > 0) $ret .= ' checked> Owner<br><br>';
                        else $ret .= '> Owner<br><br>';
                              
                    
                    

                    $ret .= '
                    <input type="input" class="form-control" name="card_no" required placeholder="Owner Number"';
                    if ($_GET['card_no'] > 0) $ret .= 'value="' . $_GET['card_no'] . '" '; 
                              $ret .= '><br><br>
        
                    <input type="input" class="form-control" name="name" placeholder="Name"';
                    if ($_GET['name']) $ret .= 'value="' . $_GET['name'] . '" '; 
                              $ret .= ' required><br><br>
              
                    <br>
                    <input type="input" class="form-control" name="phone" placeholder="Phone Number"';
                    if ($_GET['phone']) $ret .= 'value="' . $_GET['phone'] . '" '; 
                              $ret .= ' required>
                    <input type="input" class="form-control" name="alt_phone" placeholder="Alternate Number"';
                    if ($_GET['alt_phone']) $ret .= 'value="' . $_GET['alt_phone'] . '" '; 
                              $ret .= '><br><br>
         
                    <input type="input" class="form-control" name="email" placeholder="Email Address"';
...
<div class="panel panel-default smpanel"><div class="panel-heading" align="center"><b>Fog City Deli Sandwich Platters</b><br><br><i>Choose up to 2 options per platter. No 
        ...
        $ret .= '
                ...
                <div class="options">
                    ...
                    <input type="checkbox" value="Swiss & Dijon" name =\'op3011\'';
        ...
        $ret .= '
                ...
                <div class="options">
                    ...
                    <input type="checkbox" value="B.L.T." name =\'op3031\'';
                    if ($_GET['op3034'] == 'B.L.T.') $ret .= 'checked="checked" ';
                      $ret .= '> B.L.T.<br>
                    <input type="checkbox" value="Maple Chicken Wild Rice" name =\'op3032\'';
                    if ($_GET['op3035'] == 'Maple Chicken Wild Rice') $ret .= 'checked="checked" ';
                      $ret .= '> Maple Chicken Wild Rice<br>
                    <input type="checkbox" value="Greek Veggie" name =\'op3033\'';
                    if ($_GET['op3036'] == 'Greek Veggie') $ret .= 'checked="checked" ';
                      $ret .= '> Greek Veggie<br>
                    <input type="checkbox" value="Veggie Delux" name =\'op3031\'';
...
<div class="panel panel-default smpanel"><div class="panel-heading" align="center"><b>Build-Your-Own Sandwich Platters</b><br><br><i>A variety of sandwich toppings, including vegetables and condiments. Assorted bread, rolls and wraps. Vegan and wheat free available upon request.</i></div>     
        ...
        $ret .= '
                ...
                <div class="options">
                    ...
                    if ($_GET['op3073'] == 'Roast Beef') $ret .= 'checked="checked" ';
                      $ret .= '> Roast Beef<br>
                    <input type="checkbox" value="Sliced Swiss" name =\'op3071\'';
                    if ($_GET['op3074'] == 'Sliced Swiss') $ret .= 'checked="checked" ';
                      $ret .= '> Sliced Swiss<br>   
                    <input type="checkbox" value="Sharp Cheddar" name =\'op3072\'';
                    if ($_GET['op3075'] == 'Sharp Cheddar') $ret .= 'checked="checked" ';
                      $ret .= '> Sharp Cheddar<br>
                    <input type="checkbox" value="Provolone" name =\'op3073\'';
                    if ($_GET['op3076'] == 'Provolone') $ret .= 'checked="checked" ';
                      $ret .= '> Provolone<br>
                    <input type="checkbox" value="Tomato" name =\'op3071\'';
                    if ($_GET['op3077'] == 'Tomato') $ret .= 'checked="checked" ';
                      $ret .= '> Tomato<br>   
                    <input type="checkbox" value="Lettuce" name =\'op3072\'';
                    if ($_GET['op3078'] == 'Lettuce') $ret .= 'checked="checked" ';
                      $ret .= '> Lettuce<br>
                    <input type="checkbox" value="Onion" name =\'op3073\'';
        ...
        $ret .= '
                ...
                <div class="options">
                    ...
                    if ($_GET['op3093'] == 'Feta') $ret .= 'checked="checked" ';
                      $ret .= '> Feta<br>
                    <input type="checkbox" value="Cucumber" name =\'op3091\'';
                    if ($_GET['op3094'] == 'Cucumber') $ret .= 'checked="checked" ';
                      $ret .= '> Cucumber<br>   
                    <input type="checkbox" value="Shredded Carrot" name =\'op3092\'';
                    if ($_GET['op3095'] == 'Shredded Carrot') $ret .= 'checked="checked" ';
                      $ret .= '> Shredded Carrot<br>
                    <input type="checkbox" value="Falafel" name =\'op3093\'';
                    if ($_GET['op3096'] == 'Falafel') $ret .= 'checked="checked" ';
                      $ret .= '> Falafel<br>
                    <input type="checkbox" value="Shredded Beets" name =\'op3091\'';
                    if ($_GET['op3097'] == 'Shredded Beets') $ret .= 'checked="checked" ';
                      $ret .= '> Shredded Beets<br>   
                    <input type="checkbox" value="Hummus" name =\'op3092\'';
                    if ($_GET['op3098'] == 'Hummus') $ret .= 'checked="checked" ';
                      $ret .= '> Hummus<br>
                    <input type="checkbox" value="Onion" name =\'op3093\'';
        ...
        <div role="tabpanel" class="tab-pane" id="entrees">
            <h2>Entrees</h2>
                <div class="container">
                    <i>All Entrees available hot or cold. Please specify when ordering. Vegan and made
                    without wheat available upon request.</i><br>
                </div>
        ...
        if (!$_GET['card_no']) $ret .= '
            <div class="alert alert-danger" align="center">
                <b>Click</b> <i>Update Order</i> to review order.<br>
                <br>
        ...
        foreach ($_GET as $key => $qty) {
            if ($key == 'card_no') {
                $ret .= '
                    <div class="row">
                        <div class="col-xs-2"><b>Owner Number: </b></div>
                ...
                ';
            } elseif ($key == 'alt_phone') {
                $ret .= '
                    <div class="row">
                        <div class="col-xs-2"><b>Alt. Phone: </b></div>
                        <div class="col-xs-2">' . $qty . '</div>
                    </div>
                ';
            } elseif ($key == 'email') {
        ...
        */
        ...
        $ret .= '
            ...
            if ($_GET['notes']) $ret .= 'value="' . $_GET['notes'] . '" '; 
                              $ret .= '></textarea><br><br>
    ...
    public function get_view()
    {
        ...
        while($row = $dbc->fetch_row($result)){
            ...
            $ret .= '<td>new order</td>';
            //$ret .= '<td><input type="hidden" name="review" value="' . $row['order_num'] . '"></td>';
