...
class FannieReportPage extends FanniePage {
	...
	*/
	function render_data($data,$headers=array(),$footers=array(),$format='html'){
		...
		case 'csv':
			foreach($this->report_description_content() as $line)
				$ret .= $this->csv_line(array(strip_tags($line)));
		case 'xls':
			break;
		}
		...
		if (!empty($headers)){
			...
			case 'html':
				$ret .= '<thead>';
				$ret .= $this->html_line($headers1, True);
				$ret .= '</thead>';
				break;
			case 'csv':
				$ret .= $this->csv_line($headers1);
				break;
			case 'xls':
				break;
			}
         ...
         * $data may contain internally formatted tables with
         *  embedded headers and footer.
         */
		...
		if (!empty($footers)){
			...
			case 'html':
                ...
                // More than one footer row
                    ...
                    foreach ($footers as $footer) {
                        $ret .= $this->html_line($footer, True);
                    }
                }
				$ret .= '</tfoot>';
				break;
			case 'csv':
                // Should this be $footers?
				$ret .= $this->csv_line($data[$i]);
				break;
			case 'xls':
				break;
	...
	*/
	function html_line($row, $header=False){
		...
		if (($meta & self::META_BLANK) != 0){
			$ret = '</tbody><tbody><tr>';
            // The row has to have the correct number of cells
            //  for the JS sorting to work. Cannot colspan=99.
		...
		for($i=0;$i<count($row);$i){
			...
			elseif (is_numeric($row[$i]) && strlen($row[$i]) == 13){
                ...
                $textAlign = (preg_match('/\d\.\d\d*( %)*$/',$row[$i]))?' style="text-align:right;"':'';
            }
			$ret .= '<'.$tag.$textAlign.' colspan="'.$span.'">'.$row[$i].'</'.$tag.'>';
			$i += $span;
		}
		$ret .= '</tr>';
