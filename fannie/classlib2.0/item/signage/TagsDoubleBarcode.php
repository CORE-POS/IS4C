...
class TagsDoubleBarcode extends \COREPOS\Fannie\API\item\FannieSignage 
{
    ...
    public function drawPDF()
    {
        ...
        foreach ($data as $item) {
            ...
            $desc = strtoupper(substr($item['posDescription'],0,23));
            ...
            $pdf = $this->drawBarcode($upc, $pdf, $x + 2, $y + 4, $args);
            $args['width'] = 3.5;
            $args['height'] = 0.26;
            $args['vertical'] = true;
            $pdf = $this->drawBarcode($upc, $pdf, $x + 42.5, $y+2, $args);
            ...
            if (strstr($ppu, '/') && $ppu[strlen($ppu)-1] != '/') {
                $pdf->SetX($x);
                $pdf->Cell($this->width-10,4,$ppu,0,0,'R');
            }

            $pdf->SetXY($x, $y+16);
            $pdf->SetFont($this->font,'B',24);  //change font size
            $pdf->Cell($this->width-9.5,8,$price,0,0,'R');
