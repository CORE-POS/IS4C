...
class Spreadsheet_Excel_Reader
{
    ...
    private function _parse()
    {
        $pos = 0;
        try {
            $version = $this->sheetVersion($pos);
            $substreamType = $this->sheetSubType($pos, SPREADSHEET_EXCEL_READER_WORKBOOKGLOBALS);
        } catch (Exception $ex) {
            return false;
        }

        $pos += $length + 4;
        list($code, $length) = $this->getTwoInts($pos);
            ...
            switch ($code) {
                case SPREADSHEET_EXCEL_READER_TYPE_SST:
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            list ($opcode, $conlength) = $this->getTwoInts($spos);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = $this->getOneInt($spos);
                            $spos += 2;
                        }
                        if ($extendedString) {
                            // Read in cchExtRst
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $this->parseFormatRecord($pos, $version);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_XF:
                    $indexCode = $this->getOneInt($pos+6);
                    if (array_key_exists($indexCode, $this->dateFormats)) {
                        ...
                        $isdate = FALSE;
                        if ($indexCode > 0 && isset($this->formatRecords[$indexCode])) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    $this->parseBoundaries($pos, $version);
                    break;
            }
    ...
    private function sheetVersion($pos)
    {
        $version = ord($this->data[$pos + 4]) | ord($this->data[$pos + 5])<<8;
        if (($version != SPREADSHEET_EXCEL_READER_BIFF8) && ($version != SPREADSHEET_EXCEL_READER_BIFF7)) {
            throw new Exception('Unknown sheet version');
        } else {
            return $version;
        }
    }

    private function sheetSubType($pos, $desired_type)
    {
        $substreamType = ord($this->data[$pos + 6]) | ord($this->data[$pos + 7])<<8;
        if ($substreamType != $desired_type){
            throw new Exception('Unknown sheet type');
        } else {
            return $substreamType;
        }
    }

    private function parseFormatRecord($pos, $version)
    {
        $indexCode = $this->getOneInt($pos+4);

        if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
            $numchars = $this->getOneInt($pos+6);
    ...
    private function parseBoundaries($pos, $version)
    {
    ...
    private function parseTypeDimension($spos, $version, $length)
    {
    ...
    private function parseTypeMerged($spos)
    {
        $cellRanges = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
        for ($i = 0; $i < $cellRanges; $i++) {
            $first_row =  ord($this->data[$spos + 8*$i + 2]) | ord($this->data[$spos + 8*$i + 3])<<8;
            $last_row =  ord($this->data[$spos + 8*$i + 4]) | ord($this->data[$spos + 8*$i + 5])<<8;
            $first_col =  ord($this->data[$spos + 8*$i + 6]) | ord($this->data[$spos + 8*$i + 7])<<8;
            $last_col =  ord($this->data[$spos + 8*$i + 8]) | ord($this->data[$spos + 8*$i + 9])<<8;
            if ($last_row - $first_row > 0) {
                $this->sheets[$this->sn]['cellsInfo'][$first_row+1][$first_col+1]['rowspan'] = $last_row - $first_row + 1;
            }
            if ($last_col - $first_col > 0) {
                $this->sheets[$this->sn]['cellsInfo'][$first_row+1][$first_col+1]['colspan'] = $last_col - $first_col + 1;
            }
        }
    }

    private function parseTypeRK($spos)
    {
        list($row, $column) = $this->getTwoInts($spos);
        $rknum = $this->_GetInt4d($this->data, $spos + 6);
        $numValue = $this->_GetIEEE754($rknum);
        $pair = $this->numberToStringAndRaw($numValue, $spos, $column);
        $this->addcell($row, $column, $pair['string'], $pair['raw']);
    }

    private function parseTypeLabelSet($spos)
    {
        list($row, $column) = $this->getTwoInts($spos);
    ...
    private function parseTypeMultiRK($spos)
    {
        list($row, $colFirst) = $this->getTwoInts($spos);
        ...
        for ($i = 0; $i < $columns; $i++) {
            ...
            $pair = $this->numberToStringAndRaw($numValue, $tmppos-4, $colFirst + $i);
            $tmppos += 6;
            $this->addcell($row, $colFirst + $i, $pair['string'], $pair['raw']);
        }
    }

    private function parseTypeNumber($spos)
    {
        list($row, $column) = $this->getTwoInts($spos);
        $tmp = unpack("ddouble", substr($this->data, $spos + 6, 8)); // It machine machine dependent
        $pair = $this->arrayToStringAndRaw($tmp, $spos, $column);
        $this->addcell($row, $column, $pair['string'], $pair['raw']);
    }

    private function parseTypeFormula($spos)
    {
        list($row, $column) = $this->getTwoInts($spos);
    ...
    private function parseTypeBoolean($spos)
    {
        list($row, $column) = $this->getTwoInts($spos);
    ...
    private function getOneInt($spos)
    {
        return ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
    }
    ...
    {
        try {
            $version = $this->sheetVersion($spos);
            $substreamType = $this->sheetSubType($pos, SPREADSHEET_EXCEL_READER_WORKSHEET);
        } catch (Exception $ex) {
            return -1;
        }
        ...
        while ($cont) {
            ...
            switch ($code) {
                case SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
                    $this->parseTypeDimension($spos, $version, $length);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
                    $this->parseTypeMerged($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_RK:
                case SPREADSHEET_EXCEL_READER_TYPE_RK2:
                    $this->parseTypeRK($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
                    $this->parseTypeLabelSet($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_MULRK:
                    $this->parseTypeMultiRK($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    $this->parseTypeNumber($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_FORMULA:
                case SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
                    $this->parseTypeFormula($spos);
                    break;
                case SPREADSHEET_EXCEL_READER_TYPE_BOOLERR:
                    $this->parseTypeBoolean($spos);
                    break;
    ...
    private function numberToStringAndRaw($numValue, $pos, $column)
    {
        $ret = array('string' => '', 'raw' => '');
        if ($this->isDate($pos)) {
            list($ret['string'], $ret['raw']) = $this->createDate($numValue);
        } else {
            $ret['raw'] = $numValue;
            if (isset($this->_columnsFormat[$column + 1])){
                $this->curformat = $this->_columnsFormat[$column + 1];
            }
            $ret['string'] = sprintf($this->curformat, $numValue * $this->multiplier);
        }

        return $ret;
    }

    private function arrayToStringAndRaw($arr, $pos, $column)
    {
        $ret = array('string' => '', 'raw' => '');
        if ($this->isDate($pos)) {
            list($ret['string'], $ret['raw']) = $this->createDate($arr['double']);
        } else {
            $ret['raw'] = $this->createNumber($pos);
            if (isset($this->_columnsFormat[$column + 1])){
                $this->curformat = $this->_columnsFormat[$column + 1];
            }
            $ret['string'] = sprintf($this->curformat, $ret['raw'] * $this->multiplier);
        }
    ...
    {
        if ($numValue > 1) {
            return $this->integerDate($numValue);
        } else {
            return $this->floatDate($numValue);
        }
    }

    private function integerDate($numValue)
    {
    ...
    private function floatDate($numValue)
    {
        $raw = $numValue;
...
//end of changes by mmp
    ...
    {
        return $this->_ole->GetInt4d($data, $pos);
    }
