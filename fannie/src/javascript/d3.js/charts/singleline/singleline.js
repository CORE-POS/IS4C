...
function singleline(data, xrange, yrange, selector) 
{
    ...
    var val = 15000;
    if (val > 1000) {
        var b = Math.round(val/1000.00); + "k"
    }

    // create left yAxis
    var yAxisLeft = d3.svg.axis().scale(y).ticks(4).orient("left")
        .tickFormat(function(d) { if (d > 1000) { return Math.round((d/1000.00)*100)/100 + "k"; } else { return d; } });
...
function addsingleline(data, xrange, yrange, selector, color)
{
    var m = [80, 80, 80, 80]; // margins
    var w = 650 - m[1] - m[3]; // width
    var h = 400 - m[0] - m[2]; // height
    ...
    // Add the line by appending an svg:path element with the data line we created above
    // do this AFTER the axes above so that the line is above the tick-lines
    var graph = d3.select(selector+' .singleline-chart');
