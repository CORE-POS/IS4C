...
class VPBPV extends FannieRESTfulPage
{
...
<div class="container" style="user-select: none;">
    ...
    <div class="row">
        <div class="col-lg-12">
            <div align="left">
                <form name="pc-type-form" action="VPBPV.php" method="get">
                <table>
...
JAVASCRIPT;
    ...
    public function getTable()
    {
        ...
        $costSQL = Margin::adjustedCostSQL('CASE 
        ...
        $aliasP = $dbc->prepare("
            SELECT 
            CASE
                WHEN f.srp IS NOT NULL THEN f.srp
                ELSE v.srp
            END AS srp,
                v.vendorDept,
                a.multiplier
            FROM VendorAliases AS a
                ...
                LEFT JOIN FutureVendorItems AS f ON f.sku=v.sku AND f.vendorID=v.vendorID AND f.startDate > DATE(NOW())
            WHERE a.upc=?");
        ...
        $query = "SELECT p.upc,
            ...
            " . Margin::toMarginSQL($costSQL, 'CASE
                WHEN f.srp IS NOT NULL THEN f.srp
                ELSE v.srp
            END ') . " AS desired_margin,
            ...
            CASE
                WHEN f.futureCost THEN 'FutureVendor'
                ELSE 'productCost'
            END AS diffType,
            ...
            CASE
                WHEN f.futureCost THEN f.startDate
                ELSE c.date
            END AS date,
            # c.date,

            CASE
                WHEN f.startDate IS NOT NULL THEN f.startDate
                ELSE r.reviewed
            END AS reviewed,
            v.sku,
            ...
            FROM products AS p
                ...
                LEFT JOIN FutureVendorItems AS f ON f.upc=p.upc AND f.vendorID=v.vendorID AND f.startDate > DATE(NOW())
        ...
        while ($row = $dbc->fetch_row($result)) {
            ...
            */
            ...
            $priceRuleTypeID = $row['priceRuleTypeID'];
            ...
            */
            ...
            $reviewD = new DateTime($row['reviewed']);
            //if (substr($date,0,4) < substr($row['reviewed'],0,4)) {
            //    $changeClassB = 'highlight';
            //}
            $d1 = new DateTime();
            $d2 = new DateTime($row['reviewed']);
            if ($d1 < $d2) {
                $changeClassB = 'highlight-blue';
                $changeClassA = 'highlight-blue';
            }
            /*
                ...
                $td .= sprintf("<tr id=row%s class='%s %s item'>
                    ...
                    $row['upc'], $row['adjusted_cost'], 100*$row['current_margin'], 100*$row['margin'], 
                        ...
                        */
                        ...
                        $symb, round($row['difference'],2), $cleanDate,
