...
while(!feof($fp)){
	...
	$brand = 'NATRUAL FACTORS';
	$descs = explode(" ",trim($data[$DESCRIPTION]));
	$qty = 1;
	$size = "";
	$description = "";
	if(is_numeric($descs[count($descs)-1])){
		for($i=0;$i<count($descs)-2;$i++)
			$description .= $descs[$i]." ";
		$size = $descs[count($descs)-1]."-";
		$size .= strtoupper($descs[count($descs)-2]);
	}
	else {
...
if (count($filestoprocess) == 0){
	...
	echo "Finished processing NFACTOR price file<br />";
	if ($PRICEFILE_USE_SPLITS){
	...
	header("Location: loadNFACTORprices.php?filestoprocess=$sendable_data&processed=$encoded2");
