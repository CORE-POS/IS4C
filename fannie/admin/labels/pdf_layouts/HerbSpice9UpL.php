...
class HerbSpice9UpL_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function HerbSpice9UpL($data,$offset=0,$showPrice=0)
{
    $dbc = FannieDB::get(FannieConfig::config('OP_DB'));
    $pdf = new HerbSpice9UpL_PDF('L','mm','Letter');
    $pdf->AddPage();
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateHerbSpice9UpLLabel($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset, $tagNo);
        }
...
function generateHerbSpice9UpLLabel($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset, $tagNo)
{
    $x += 5;
    $pdf->SetFont('Gill','', 16);
    ...
    */
    ...
    $pdf->SetXY($x+$width-30,$y+5.4);
    $pdf->Cell(10, 2, $vendor, 0, 1, 'R', true);
    $pdf->SetXY($x+$width-30,$y+7.4);
    $pdf->Cell(10, 2, $sku, 0, 1, 'R', true);
    // Add Movement
    $pdf->SetXY($x+$width-30,$y+3.4);
