...
class WFC_New_MEAT_14UP_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function WFC_New_MEAT_14UP($data,$offset=0,$showPrice=0)
{
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateNewMeat_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset, $tagNo);
    ...
    */
    ...
    $data = arrayMirrorRowsNewMeat_24UP($data, 2);
    $pdf->AddPage('L');
    foreach($data as $k => $row){
        if ($i % 14 == 0 && $i != 0) {
            $pdf->AddPage('L');
        ...
        if ($i == 0) {
            $pdf = generateMirrorNewMeatTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 2 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function arrayMirrorRowsNewMeat_24UP($array, $cols)
{
    $newArray = array();
