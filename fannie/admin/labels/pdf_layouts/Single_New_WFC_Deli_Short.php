...
class Single_New_WFC_Deli_Short_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function Single_New_WFC_Deli_Short($data,$offset=0,$showPrice=0)
{
    $dbc = FannieDB::get(FannieConfig::config('OP_DB'));
    //$pdf = new Single_New_WFC_Deli_Short_PDF('L','mm','Letter');
    $pdf = new Single_New_WFC_Deli_Short_PDF('L', 'mm', array(105, 148));
    $pdf->AddPage("L", "A6");
    ...
    $mirrorData = arrayMirrorRowsSingleNewDeliShort_24UP($data, 2);
    ...
    foreach($data as $k => $row){
        ...
        if ($i % 4 == 0 && $i != 0) {
            ...
            prepSingleNewDeliShortMirrorTags($start, $tagData, $pdf, $width, $height, $left, $top, $guide, $dbc);
        ...
        if ($i == 0) {
            $pdf = generateSingleNewDeliShort_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset);
        } else if ($i % 2 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide+5;
        } else {
...
function prepSingleNewDeliShortMirrorTags($start, $data, $pdf, $width, $height, $left, $top, $guide, $dbc)
{
    /*
    ...
    */
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateNewDeliSingleShortMirrorTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 2 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide+5;
        } else {
...
function generateNewDeliSingleShortMirrorTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc)
{
...
function generateSingleNewDeliShort_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset)
{
    $upc = $row['upc'];
...
function arrayMirrorRowsSingleNewDeliShort_24UP($array, $cols)
{
    $newArray = array();
