...
/*
    @class WFC_Deli_Narrow_24UP_PDF
    1/2 Width Deli (Dark Extended) Tags
*/
class WFC_Deli_Narrow_24UP_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function WFC_Deli_Narrow_24UP($data,$offset=0,$showPrice=0)
{
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateNarrow_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset);
        } else if ($i % 5 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function arrayMirrorRowsNarrow_24UP($array, $cols)
{
    $newArray = array();
