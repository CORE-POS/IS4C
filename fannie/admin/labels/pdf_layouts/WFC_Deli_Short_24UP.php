...
/*
    @class WFC_Deli_Short_24UP_PDF
    A Deli (Dark Extended) alternate that prints a few millimeters shorter to
    fit certain slightly smaller rails.
*/
class WFC_Deli_Short_24UP_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function WFC_Deli_Short_24UP($data,$offset=0,$showPrice=0)
{
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateShort_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset);
        } else if ($i % 4 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
    ...
    */
    ...
    $data = arrayMirrorRowsShort_24UP($data, 4);
    $pdf->AddPage('L');
    foreach($data as $k => $row){
        if ($i % 24 == 0 && $i != 0) {
            $pdf->AddPage('L');
        ...
        if ($i == 0) {
            $pdf = generateDeliShortMirrorTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 4 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function arrayMirrorRowsShort_24UP($array, $cols)
{
    $newArray = array();
