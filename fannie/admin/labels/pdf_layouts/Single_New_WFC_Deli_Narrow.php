...
class Single_New_WFC_Deli_Narrow_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function Single_New_WFC_Deli_Narrow($data,$offset=0,$showPrice=0)
{
    ...
    $mirrorData = arrayMirrorRowsSingleNewDeliNarrow_24UP($data, 2);
    ...
    foreach($data as $k => $row){
        ...
        if ($i % 4 == 0 && $i != 0) {
            ...
            prepSingleNewDeliNarrowMirrorTags($start, $tagData, $pdf, $width, $height, $left, $top, $guide, $dbc);
...
function prepSingleNewDeliNarrowMirrorTags($start, $data, $pdf, $width, $height, $left, $top, $guide, $dbc)
{
    /*
    ...
    */
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateNewDeliSingleNarrowMirrorTag($x+35, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 2 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function generateNewDeliSingleNarrowMirrorTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc)
{
...
function arrayMirrorRowsSingleNewDeliNarrow_24UP($array, $cols)
{
    $newArray = array();
