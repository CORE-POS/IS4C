...
function WFC_Dark_HFM_12UP ($data,$offset=0)
{
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateHFMTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 4 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
    ...
    */
    ...
    $data = arrayMirrorRowsHFM($data, 4);
    $pdf->AddPage('L');
    foreach($data as $k => $row){
        if ($i % 12 == 0 && $i != 0) {
            $pdf->AddPage('L');
        ...
        if ($i == 0) {
            $pdf = generateMirrorTagHFM12($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 4 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function arrayMirrorRowsHFM($array, $cols)
{
    $newArray = array();
