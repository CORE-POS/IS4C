...
class WFC_MEAT_SN_14UP_PDF extends FpdfWithBarcode
{
    private $tagdate;
...
function WFC_MEAT_SN_14UP($data,$offset=0,$showPrice=0)
{
    ...
    foreach($data as $k => $row){
        ...
        if ($i == 0) {
            $pdf = generateMeat_SN_24UPTag($x, $y, $guide, $width, $height, $pdf, $row, $dbc, $showPrice, $offset, $tagNo);
    ...
    */
    ...
    $data = arrayMirrorRowsMeat_SN_24UP($data, 2);
    $pdf->AddPage('L');
    foreach($data as $k => $row){
        if ($i % 14 == 0 && $i != 0) {
            $pdf->AddPage('L');
        ...
        if ($i == 0) {
            $pdf = generateMirrorMeatTagSN($x, $y, $guide, $width, $height, $pdf, $row, $dbc);
        } else if ($i % 2 == 0 && $i != 0) {
            $x = $left+$guide;
            $y += $height+$guide;
        } else {
...
function arrayMirrorRowsMeat_SN_24UP($array, $cols)
{
    $newArray = array();
