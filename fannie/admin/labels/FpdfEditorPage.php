...
class FpdfEditorPage extends FannieRESTfulPage 
{

    protected $header = 'FPDF Layout Editor';
    protected $title = 'Edit FPDF Classes';
    public $description = '[FPDF Layout Editor] save PDF output to single file
        for quick viewing & revision.';
    private $items = array();
    ...
    public function get_view()
    {
        ...
        $ret .= '<label>Layout</label>: 
            <select name="signmod" id="signmod" class="form-control" >
                <option value="Legacy:FPDF EditPage" selected>Legacy:FPDF EditPage</option>
                <option value="Legacy:WFC Aisle Tags">Legacy:WFC Aisle Tags</option>
                ';
        $ret .= '</select>';
        $ret .= '&nbsp;&nbsp;&nbsp;&nbsp;';
        $ret .= '<span  name="pdf" value="Print" 
                    class="btn btn-default" id="Print">Print</span>
...
JAVASCRIPT;
        ...
        $ret .= '</tbody></table>';
        $ret .= '<input type="hidden" name="form_page" value="FpdfEditorPage" />';
...
$('#Print').on('click', function(){
    document.forms['signform'].target = "_blank";
...
JAVASCRIPT;
        ...
        $this->addOnloadCommand($printJS);

        return <<<HTML
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-6">$ret</div>
        <div class="col-lg-6">
            <embed src="noauto/Output.pdf" width="100%" height="900px" />
        </div>
    </div>
</div>
HTML;
