<?php
	...
	if (!isset($_REQUEST['id'])) {
		header ("Location: {$FANNIE_URL}batch/");
	}
	...
	if (isset($_REQUEST['a']) && $_REQUEST['a']=='editBatch') {
		...
		if ($search_batchProducts_result) {
			if ($dbc->num_rows($search_batchProducts_result)==1) {
				$search_resultMatched=$search_batchProducts_result;
			} else if ($dbc->num_rows($search_batchProducts_result)>1) {
				...
				if ($search_allProducts_result) {
					if ($dbc->num_rows($search_allProducts_result)==1) {
						$search_resultMatched=$search_allProducts_result;
					} else if ($dbc->num_rows($search_allProducts_result)>1) {
						$search_result=$search_allProducts_result;
					} else {
						array_push($backoffice['status'], 'No results found for '.$_REQUEST['searchProduct_upc']);
					}
				} else {
					array_push($backoffice['status'], 'batchProducts_result==0, allProducts_result error ('.$dbc->error().')');
				} 
			}
		} else {
			// TODO - Maybe still try to search products table?
			array_push($backoffice['status'], 'batchProducts_result error('.$dbc->error().')');
		}
	} else if (isset($_REQUEST['a']) && $_REQUEST['a']=='addProduct') {
		addProduct(&$backoffice);
	}
	
	require_once($FANNIE_ROOT.'lib/materialized_batch.php');
		$batchInfo_result=get_batchInfo(&$backoffice, $_REQUEST['id']);
		$batchInfo_row=$dbc->fetch_row($batchInfo_result);
...
<html>
	...
	while ($row=$dbc->fetch_array($batchTypes_result)) {
		$html.='
				...
				<form action="./edit.php" method="post" name="searchResults" onsubmit="return false;">
					<fieldset>
						...
						<table>
							...
							<tfoot/>
							<tbody>';
		while ($row=$dbc->fetch_array($search_result)) {
			$html.='
								<tr>
	...
	if (isset($search_resultMatched)) {
		$row=$dbc->fetch_array($search_resultMatched);
		$html.='
			<div>
	...
	if ($dbc->num_rows($batchProducts_result)>0) {
		$html.='
						<table>
							<thead>
								<tr>
		...
		while ($row=$dbc->fetch_array($batchProducts_result)) {
			$html.='
								<tr>
									<td><a href="edit.php?a=searchProduct&id='.$_REQUEST['id'].'&searchProduct_upc='.$row['upc'].'">'.$row['upc'].'</a></td>
									<td>'.$row['description'].'</td>
